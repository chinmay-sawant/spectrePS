...
- [x] `verapdf --flavour ua2 --format json` outcomes are recorded on the rows for the fixtures. veraPDF is a proof tool, never a build or runtime dependency. Proof: the recorded command and outcome. Outcomes 2026-09-25 with veraPDF 1.30.2 on PATH: `make pdfua2-check` ran `verapdf --flavour ua2 --format json` over `sampledata/pdfua2/tagged-ua2.pdf` and `sampledata/pdfua2/compliant-ua2.pdf` and exited 0. veraPDF reported 1727 passed rules and 0 failed rules for each file. The earlier run had failed `tagged-ua2.pdf` on clause 8.11.1 test 2 because the XMP stream dictionary had no `/Type /Metadata` and no `/Subtype /XML`; `internal/pdfa/gen_ua2_samples.go` now writes both, and the sample was regenerated. The `negative/untagged.pdf` fixture fails `ua2-marked` by design and is excluded from the make run by the `*/negative/*` filter. The copied sample is `sampledata/wasm-js/compliant.pdf` from the local gopdfsuit project, SHA-256 `475526f82afe9eb6e36f702910c538fc956fbd8bf658e4183efbdceca74e870c`, MIT licensed, same author. `sampledata/pdfua2/README.md` records both files. The samples are written by `go run internal/pdfa/gen_ua2_samples.go` and stable across runs.
