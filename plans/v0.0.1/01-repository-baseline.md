...
The module path is `github.com/chinmay-sawant/spectrePS`. The `go` line is `1.26.4`. Docs under `documentation/` are the behavior source. Plans under `plans/v0.0.1/` are the ledger. `skills/unslop/SKILL.md` is the local copy of the writing skill.
...
- [x] `documentation/README.md` indexes the contracts.
- [x] `documentation/ghostscript-baseline.md` records local `gs` 9.55.0 and the 10.08 manual split between raster, rewrite, validate, and compare.
- [x] `documentation/architecture.md` records the instance, the two front ends, and the device seam.
- [x] `documentation/folder-structure.md` records the tree and the import rule.
- [x] `documentation/public-api.md` records exported signatures.
- [x] `documentation/cli.md` records subcommands and exit codes.
- [x] `documentation/language.md` records the PostScript subset, errors, limits, and banned operators.
- [x] `documentation/devices.md` records pixmap layout, PPM, PNG, compare, rewrite, and the PDF operator subset.
- [x] `documentation/development.md` records make targets and the checklist rule.
