# Spectre PS pull request template

Use this document when authoring GitHub pull requests for [chinmay-sawant/spectrePS](https://github.com/chinmay-sawant/spectrePS). Copy the sections below into the PR description and fill in each section. Delete guidance comments before submitting.
...
- [ ] `make test`
- [ ] `make lint`
- [ ] `make build` when the change adds or edits Go code under `cmd/spectreps`
...
```
feat(ps): execute the stack subset
fix(raster): flip y at the pixmap device
docs: record the pdf subset
chore: refresh the v0.0.1 ledger
```
...
gh pr create \
  --base master \
  --title "feat(ps): execute the stack subset" \
