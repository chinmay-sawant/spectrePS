...
- `gs` resolves a name in `/Resources /ExtGState`, applies `/LW`, and restores the width on `Q`. An unknown name and an unsupported entry both return `undefined in gs`, so the state is never partly applied, and a non-number `/LW` is `typecheck`.
