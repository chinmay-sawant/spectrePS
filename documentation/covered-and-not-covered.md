...
- Select pages with `-pages`, in the style of `-dFirstPage` and `-dLastPage`. `raster`, `bbox`, `inkcov`, `ink_cov`, `pdfimage`, and `compare raster` take the flag.
...
- Paint transparency and optional content. Spectre paints `/ExtGState` `/CA`, `/ca`, `/BM`, and `/SMask`, transparency groups, image masks, color-key and stencil `/Mask`, and `/Decode` arrays, and it skips content under an OFF optional content group or XObject. The default `/AIS`, `/OP`, and `/op` flags are accepted as no-ops with `/OPM` and `/SA`, and a true overprint or alpha-is-shape flag refuses. The default OCG configuration is read; alternates stay out.
...
- Preserve an existing PDF/UA-2 structure tree through a pass-through rewrite, generate one for an untagged PDF, and preflight the result. Levels 1 through 5 keep `/StructTreeRoot`, MCIDs, `/Alt`, `/ActualText`, and `/Lang`; level 0 and `pdfimage` return `/tagged` instead. `rewrite -tags` derives a reading order from device geometry, writes `/P`, `/H1` through `/H6`, `/L` with `/Lbl` and `/LBody`, `/Table` with `/Scope`, `/Figure` with `/Alt` from the source, narrow `/ActualText`, and `/Artifact` for rules and repeated furniture, and runs the UA-2 preflight before it writes `pdfuaid`. The reader accepts `BMC`, `BDC`, `EMC`, `MP`, and `DP`, so a tagged page rasterizes and extracts with the markers absent. The UA-2 preflight reports `Error: /ua2-<rule> in PDFUA`, and the result is generate and preflight, never certification.
- Run a PDF/UA-2 content-side check from `validate` for a tagged input, so a structure tree that disagrees with its marked content fails the command, in the spirit of a validation profile. The check is the machine subset, not a certificate.
...
- Full PDF 1.7 and PDF 2.0. Alternate optional content configurations, the `/AS` usage map, and the visibility flag stay out. Encryption and passwords stay out. The soft mask, transparency group, dash pattern, text rendering mode, overprint, and stroke adjustment no-ops are recorded in `documentation/devices.md`.
...
- PDF/UA-2 certification, PAC reliance, and human verification. Spectre generates and preflights the machine-checkable subset from device geometry; the reading order is an approximation and the result is not a certificate.
- PDF/X creation.
