...
| `bind`, `save`, `restore`, `clip`, and filters other than Flate | The PostScript operators stay out. PDF `W` and `W*` read and clip the pixmap, but the level 0 recorder implements no clip seam and keeps refusing them. The remaining stream filters (LZW, ASCII85, ASCIIHex, RunLength, and Flate predictors) are planned. | `plans/v0.0.4/4-pdf-coverage.md`. |
