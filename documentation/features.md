...
| Text extraction, `show`, `Tj` | Fonts are a separate machine from the path engine. | `plans/v0.0.3/9-text-and-fonts.md`. |
| PDF/UA-2 preflight completion | The structure model, parent tree, role map, and pass-through preservation landed. Metadata read/write and the machine checks wait on the PDF/A metadata writer, and tag generation needs the text and font machine. | `plans/v0.0.3/10-pdfua2.md`. |
