...
`PDF version` is the higher of the header version and the current catalog's `/Version` name. A catalog replaced by an incremental update can raise it. `Pages` is the page tree leaf count, and each `Page` line is the resolved `/MediaBox` width and height in points, inherited from the nearest `/Pages` ancestor and defaulting to 612 by 792 when the tree has none. `Tagged` is the reader tagged flag. The `Fonts:` block lists every in-use `/Type /Font` dictionary except CIDFont descendants, sorted by name, and `embedded=true` means a `/FontFile`, `/FontFile2`, or `/FontFile3` program is in the file. A file with no fonts prints `Fonts: none`. `Images` counts the in-use image XObjects. An encrypted trailer exits 1 with `Error: /invalidaccess in Encrypt`, because the reader refuses it. A missing input or a bad flag exits 2, an unreadable path exits 3, and a malformed document exits 1. This report does not check PDF conformance; `documentation/pdf-compatibility.md` records the distinction.
