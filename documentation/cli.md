...
`raster` writes a PPM raw file (P6) unless `-o` ends in `.png`, `.jpg`, or `.jpeg`. A PDF input paints every selected page with `RasterizePage`; any other input uses `RunPostScript`.
