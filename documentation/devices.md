...
- `/CCITTFaxDecode` with `/DeviceGray` at 1 bit per component. `/K < 0` is Group 4 and `/K == 0` with `/EndOfLine true` is Group 3, both through `golang.org/x/image/ccitt`. `/K == 0` without end-of-line markers and `/K > 0` mixed one- and two-dimensional Group 3 decode in `internal/pdf`, and every `/K > 0` row starts with an end-of-line marker and a tag bit. `/Columns` and `/Rows` default to `/Width` and `/Height`, `/BlackIs1` inverts the samples, and `/EncodedByteAlign` byte-aligns the codes. The decoded rows stop at `/Rows`, so `/EndOfBlock false` decodes without a closing pattern. A row cut off mid-code is `syntaxerror`, and `Columns * Rows` above the 32 MiB decoded cap returns `limitcheck` before allocation.
...
`PreflightUA2` is the UA-2 request, separate from the PDF/A preflight. It returns `Error: /ua2-<rule> in PDFUA` for `ua2-marked`, `ua2-structtree`, `ua2-document`, `ua2-content`, `ua2-lang`, `ua2-displaydoctitle`, `ua2-pdfuaid`, `ua2-title`, `ua2-rolemap`, and `ua2-mcid`. A PDF/A-only problem such as an LZW stream or a non-embedded font does not fail the UA-2 request, and the UA-2 checks never run for a PDF/A request. The claim is preflight only, never certification.

`ua2-content` is the MCID coverage check. It decodes every page and proves coverage in both directions: a marked-content item in the content with no parent tree claim is uncovered, and a parent tree claim with no content item is uncovered too. It runs its own content scanner rather than the shared interpreter, so a malformed marked-content sequence fails here and not silently.
...
The program starts with `%!PS-Adobe-3.0` and a `%%BoundingBox` taken from the first page's resolved `/MediaBox`, so the header names the document's own page rather than a fixed letter box. The box is floored at the minimum and ceiled at the maximum, because DSC wants integers, which is the same rule the `bbox` command uses. A page with no resolvable box falls back to `0 0 612 792`. The box comes from the first page because a DSC program carries one. A prolog defines the short path names in terms of `moveto`, `lineto`, `stroke`, `fill`, and `eofill`, because a bare `m` or `S` is not a PostScript operator. Each page gets a `%%Page` comment and one `showpage`, and the program ends with `%%EOF`. There is no creation date, and two calls on the same document return equal buffers, so `CompareFiles` is the proof.
