...
func TestTintTransformSampled(t *testing.T) {
	...
	tint := DictVal(map[string]Value{
		...
		keyDomain:       numberArray(0, 1),
		keyC0:           numberArray(0, 0, 0, 0),
		keyC1:           numberArray(1, 1, 1, 0),
		keyN:            RealVal(1),
	})
	resolved := colorSpaceFor(t, nil,
...
func numberArray(values ...float64) Value {
...
func calculatorValue(inputs int, code string) Value {
	...
	return StreamVal(map[string]Value{
		...
		keyDomain:       numberArray(domain...),
		keyRange:        numberArray(0, 1, 0, 1, 0, 1, 0, 1),
	}, []byte(code))
}
